...
#ifndef ROO_CMS_SHAPE2
#define ROO_CMS_SHAPE2
...
class RooBkgShape : public RooAbsPdf {
public:
  RooBkgShape() {};
  RooBkgShape(const char *name, const char *title,
	      RooAbsReal& _x,
	      RooAbsReal& _alpha,
	      RooAbsReal& _beta,
	      RooAbsReal& _gamma,
  ...
  RooBkgShape(const RooBkgShape& other, const char* name);
  inline virtual TObject* clone(const char* newname) const { return new RooBkgShape(*this,newname); }
  inline ~RooBkgShape() {}
  Double_t evaluate() const ;
  

  ClassDef(RooBkgShape,1);
